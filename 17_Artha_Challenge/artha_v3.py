...
def rms(values):
    ...
    return math.sqrt(
        sum(x * x for x in clean) / len(clean)
    )
...
class MyBot(Bot):
    ...
    def estimate_liquidity(self, state, sym):
        ...
        last_volume = safe_number(
            state.last_volume.get(sym, 0.0)
        )
        ...
        expected_volume = geometric_blend(
            full_level,
            recent_level
        )
    ...
    def regime_factor(self, state, sym):
        ...
        returns = self.return_history.get(
            sym,
            []
        )
        ...
        if clean_volumes:
            ...
            recent_volume = robust_location(
                clean_volumes[-recent_n:]
            )

            all_volume = robust_location(
                clean_volumes
            )
        ...
        if returns:
            ...
            recent_n_ret = self.recent_window_length(
                n_ret
            )

            recent_volatility = rms(
                returns[-recent_n_ret:]
            )
    ...
    def completion_fraction(self, state, sym):
        ...
        mandate = safe_number(
            state.mandate.get(sym, 0.0)
        )

        remaining = safe_number(
            state.remaining.get(sym, 0.0)
        )
    ...
    def on_bar(self, state):
        ...
        for sym in active:
            ...
            completion[sym] = self.completion_fraction(
                state,
                sym
            )
        ...
        for sym in active:
            ...
            expected_volume, _volume_surprise = (
                self.estimate_liquidity(
                    state,
                    sym
                )
            )

            regime_quality = self.regime_factor(
                state,
                sym
            )

            expected_bar_capacity = (
                ENGINE_PARTICIPATION_CAP
                *
                expected_volume
                *
                regime_quality
            ...
            if (
                ...
                or adaptive_pressure <= 0.0
            ):

                adaptive_pressure = 1.0

            adaptive_multiplier = math.sqrt(
                adaptive_pressure
            )
