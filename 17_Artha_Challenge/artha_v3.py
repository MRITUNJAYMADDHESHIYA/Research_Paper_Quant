   ...
   ############## Bar ################
    def on_bar(self, state):
        ...
        for sym in active:
            ...
            if (
                not math.isfinite(feasibility_pressure)
                or feasibility_pressure <= 0.0
            ):
                feasibility_pressure = 1.0

            # Current-bar opportunity only.
            # Do not multiply regime_quality into this again; it is already
            # represented in the future-capacity / feasibility calculation.
            ...
            if (
                not math.isfinite(adaptive_pressure)
                or adaptive_pressure <= 0.0
            ):
                adaptive_pressure = 1.0

            adaptive_multiplier = math.sqrt(adaptive_pressure)
            desired = required_rate * adaptive_multiplier

            # Completion safety.  This activates only when the remaining
            # inventory can no longer fit inside estimated future capacity.
