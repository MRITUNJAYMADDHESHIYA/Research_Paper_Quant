...
class BacktestEngine:
    ...
    def run(self, actual_returns, signals):
        actual_returns   = np.asarray(actual_returns)
        signals          = np.asarray(signals)
        position         = signals                                  ####### position
        strategy_returns = (position * actual_returns)              ####### strategy return
        position_change  = np.abs(np.diff(np.insert(position,0,0))) #### Transaction cost
    ...
    def metrics(self, result):
        ...
        trades       = (np.abs(np.diff(result["Signal"])) > 0).sum()
